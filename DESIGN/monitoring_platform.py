...
class MonitoringPlatform:
    ...
    def load_algorithms_from_json(self, json_path='algorithm_data.json') -> Tuple[Dict, int]:
        ...
        try:
            ...
            self.log_message(f"警告: 读取算法数据失败 - {e}")
            return {}, 0
    ...
    def launch_generic_algorithm(self, algo_path, algorithm_text, category_info):
        ...
        # 检查路径
        if not algo_path:
            messagebox.showerror("错误", f"无法找到算法 {display_name} 的文件")
            return
        ...
        try:
            # 启动进程
            self.log_message(f"启动算法: {algo_path} {port}")
            ...
            instance = {
                ...
                'cpu_usage': "",
                'gpu_usage': "",
                'is_remote': category_info.get('is_remote', False)
            }
    ...
    def get_algorithm_path(self, item_id):
        ...
        if not parent_id:
            self.log_message(f"错误: 无法获取算法 {algorithm_name} 的集成方式")
            return None

        # 获取集成方式名称，去除可能的计数部分
        integration_type = re.sub(r'\s*\(\d+\)$', '', self.tree_nodes[parent_id]["text"])

        # 构建算法目录路径
        current_dir = os.path.dirname(os.path.abspath(__file__))
        algorithm_dir = os.path.join(current_dir, "生成框架代码", integration_type, algorithm_name)

        self.log_message(f"查找算法路径: {algorithm_dir}")

        # 检查目录是否存在
        if not os.path.exists(algorithm_dir):
            self.log_message(f"错误: 算法目录不存在: {algorithm_dir}")
            return None

        # 在目录中查找唯一的Python文件
        py_files = [f for f in os.listdir(algorithm_dir) if f.endswith('.py')]

        if len(py_files) == 0:
            self.log_message(f"错误: 在目录 {algorithm_dir} 中没有找到Python文件")
            return None
        elif len(py_files) == 1:
            # 找到唯一的Python文件
            py_file = py_files[0]
            self.log_message(f"找到算法文件: {py_file}")
            return os.path.join(algorithm_dir, py_file)
        else:
            # 如果有多个Python文件，尝试找到包含算法名称的那个
            for py_file in py_files:
                if algorithm_name.lower() in py_file.lower():
                    self.log_message(f"找到匹配的算法文件: {py_file}")
                    return os.path.join(algorithm_dir, py_file)

            # 如果没找到匹配的，返回第一个Python文件
            self.log_message(f"在多个Python文件中选择第一个: {py_files[0]}")
            return os.path.join(algorithm_dir, py_files[0])
