...
class HTTPStatusReporter:
    ...
    def build_status_message(self, algorithm_name, algorithm_info):
        ...
        status_data = [{
            ...
            "className": algorithm_info.get("class", "目标检测类"),
            "subcategory": algorithm_info.get("subcategory", "深度学习检测类"),
...
def main():
    ...
    print("="*60 + "\n")
    config_param = "{'category': '外置信息代理', 'class': '认知识别类', 'subcategory': '目标检测类', 'version': '1.0.0', 'creator': '聊可好', 'create_time': '2025-04-01', 'maintainer': '好帅哦', 'update_time': '2025-11-05 14:59:33', 'description': 'DeepSeek_R1是一个大规模预训练语言模型，能够执行自然语言处理、推理、规划和决策辅助等任务。11月5日。', 'inputs': [{'name': '提示词', 'symbol': 'prompt', 'type': '字符串', 'dimension': '1', 'description': '用户输入的提示词或指令'}, {'name': '系统信息', 'symbol': 'system', 'type': '字符串', 'dimension': '1', 'description': '系统角色设定和任务描述'}, {'name': '历史对话', 'symbol': 'history', 'type': '列表', 'dimension': '1', 'description': '备注'}, {'name': '123', 'symbol': 'xyz', 'type': 'double', 'description': 'beizhu hhhhh', 'dimension': 'm'}], 'outputs': [{'name': '回复内容', 'symbol': 'response', 'type': '字符串', 'dimension': '1', 'description': '模型生成的回复内容'}, {'name': '置信度', 'symbol': 'confidence', 'type': '标量', 'dimension': '1', 'description': '模型对回复内容的置信度'}, {'name': '555', 'symbol': '', 'type': 'Any', 'description': ''}], 'network_info': {'ip': '192.168.75.136', 'port': 9090, 'status': '离线', 'last_update_timestamp': 1744015861.2297077, 'memory_usage': '2.73%', 'cpu_usage': '0.00%', 'gpu_usage': '未知', 'is_remote': True}, 'name': 'YOLO算法', 'english_name': 'YOLO', 'new_algo_name': 'def'}"
    ...
    algorithm_info = {
        ...
        "version": "1.0",
        "creator": "system",
        "description": "YOLO目标检测算法，用于实时目标检测和识别",
        "inputs": [
            {"name": "image", "type": "image", "description": "输入图像"},
            {"name": "confidence", "type": "float", "description": "置信度阈值"}
        ],
        "outputs": [
            {"name": "detections", "type": "list", "description": "检测结果列表"},
            {"name": "boxes", "type": "array", "description": "边界框坐标"},
            {"name": "scores", "type": "array", "description": "置信度分数"}
        ],
        "network_info": {
            "port": args.algo_port,
            "status": "运行中"
